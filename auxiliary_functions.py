...
#     print(df_processed['wave_id'].value_counts())# drop unused columns ['i', 'ind', 'cou', 'indust', 'o', 'sch', 'course_of']
    df_processed = df_processed.drop(['c', 'organisat', 'course', 'industrie', 'wave_id', 'i', 'ind', 'cou', 'indust', 'o', 'sch', 'course_of'], axis=1, errors='ignore')
#     df_processed.info()
...
def filter_dataframe(df):
    ...
    with c:
        ...
        for column in to_filter_columns:
            ...
            f"Enter sub filter string in {column}")
            if user_text_input:
                df = df[df[column].astype(str).str.contains(user_text_input, case=False)]
    st.write('Percentage of data that has this combination: ', len(df)/full_size*100,'%')
    return df

def pie_chart(df):

    #df_10 = df.groupby(['name']).size().to_frame().sort_values([0], ascending = False).head(10).reset_index()
    fig_pie = px.pie(df.head(10), names='count', title='Top 10 most selected industries', hole=0.4)
