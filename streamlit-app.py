...
show_pages(
    [
        ...
        Page("pages/detailed information.py", "Detailed Information", "🌐"),
    ]
)
...
with col1:
    ...
    st.metric(label="Number of unique visitors", value=f'{df.nunique()["session_id"]}', delta=f'{df_filtered.nunique()["session_id"]} in the last 24h')
...
with col2:
    ...
    value_df = pd.DataFrame({'industries':value_counts.index, 'count':value_counts.values})
    st.metric(label="Most popular filter in the past month", value=f'{mode}', delta=f'{value_counts.iloc[0]} in the past month')
...
with col3:
    ...
    fig_line = px.line(df1, x="date", y="count", title='frequency of the most popular search filter')
