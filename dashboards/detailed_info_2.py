...
with st.form("full query"):
    ...
    if query and filter_dict:
        ...
        for filter, query in filter_dict.items():
            if filter == "" or query == "":
                continue
            db_copy = cur2.execute(f"""SELECT * FROM db_copy WHERE CAST({filter} AS VARCHAR) ILIKE {"'%" + query + "%'"};""").fetch_df()
        st.write(
            "Percentage of data that has this combination: ", len(db_copy) / full_size * 100, "%"
        )
