...
# remove duplicates
grouped_df['industries'] = grouped_df['industries'].apply(lambda x: list(dict.fromkeys(x)))
...
def rolling_window_switches(grouped_df, window_minutes):
    ...
    for visit_id, group in grouped_df.groupby('visit_id'):
        ...
        while start_idx < group_len:
            ...
            while end_idx + 1 < group_len and times[end_idx + 1] - times[start_idx] <= window_delta:
                end_idx += 1
...
switches_df = pd.DataFrame([
    {'from': f, 'to': t, 'count': c}
    for (f, t), c in switches_count.items()
